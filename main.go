...
import (
	...
	raudio "github.com/shinsx/go-scroll-game/resources/audio"
	resources "github.com/shinsx/go-scroll-game/resources/images"
...
var (
	...
	congratsImage    *ebiten.Image
	arcadeFaceSource *text.GoTextFaceSource
)

func init() {
	img, _, err := image.Decode(bytes.NewReader(resources.Mofmof_png))
	...
	img, _, err = image.Decode(bytes.NewReader(resources.Congrats_png))
	if err != nil {
		log.Fatal(err)
	}
	congratsImage = ebiten.NewImageFromImage(img)
}
...
func (g *Game) Update() error {
	...
	case ModeGame:
		...
		// score が 1 以上になったらゴールに
		if g.score() >= 1 {
			g.mode = ModeGoal
		}

	case ModeGoal:
		// ゴール状態ではキャラクターの動作は停止させるなどの処理（必要なら）
		// ゴール後にキー入力でリセットする処理も追加
		if g.isKeyJustPressed() {
			g.init()
			g.mode = ModeTitle
		}
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	case ModeGameOver:
		texts = "\nGAME OVER!"
	case ModeGoal:
		// ゴール時はタイトルやスコアの表示の代わりに「おめでとう！」画像を重ね描き
		// ※ここでは例として、画像を画面中央に描画しています
		op := &ebiten.DrawImageOptions{}
		w, h := congratsImage.Bounds().Dx(), congratsImage.Bounds().Dy()
		op.GeoM.Translate(float64(screenWidth-w)/2, float64(screenHeight-h)/2)
		screen.DrawImage(congratsImage, op)
	}

	// タイトルやスコアのテキスト描画（ModeGoal 時は必要に応じて省略可能）
	if g.mode != ModeGoal {
		op := &text.DrawOptions{}
